...
func (s *mcpServerWithClientSession) ListTools(ctx context.Context, agent *Agent) ([]*mcp.Tool, error) {
	...
	tools = s.applyToolFilter(ctx, tools, agent)
	return tools, nil
}
...
// applyToolFilter applies the tool filter to the list of tools.
func (s *mcpServerWithClientSession) applyToolFilter(
	ctx context.Context,
	tools []*mcp.Tool,
	agent *Agent,
) []*mcp.Tool {
	...
	for _, tool := range tools {
		shouldInclude, err := s.toolFilter.FilterMCPTool(ctx, filterContext, tool)
		if err != nil {
