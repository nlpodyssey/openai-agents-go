...
func TestRequiredToolChoiceWithMultipleRuns(t *testing.T) {
	...
	result1, err := agents.Runner().Run(t.Context(), agents.RunParams{
		StartingAgent: agent,
		Input:         agents.InputString("first run"),
	})
	...
	result2, err := agents.Runner().Run(t.Context(), agents.RunParams{
		StartingAgent: agent,
		Input:         agents.InputString("second run"),
	})
	require.NoError(t, err)
...
func TestRequiredWithStopAtToolName(t *testing.T) {
	...
	result, err := agents.Runner().Run(t.Context(), agents.RunParams{
		StartingAgent: agent,
		Input:         agents.InputString("run test"),
	})
	require.NoError(t, err)
