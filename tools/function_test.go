...
func TestGetWeatherTool_Manual(t *testing.T) {
	GetWeatherTool := Function{
		...
		OnInvokeTool: func(ctx context.Context, _ *runcontext.Wrapper, arguments string) (any, error) {
			var args GetWeatherArgs
			err := json.Unmarshal([]byte(arguments), &args)
			if err != nil {
				return nil, err
			}
			w, err := GetWeather(ctx, args)
			if err != nil {
				return nil, err
			}
	...
	t.Run("ManualTool_Invocation", func(t *testing.T) {
		...
		rcw := &runcontext.Wrapper{}

		args := `{"city": "Tokyo"}`
		result, err := GetWeatherTool.OnInvokeTool(ctx, rcw, args)
		require.NoError(t, err)

		var weather Weather
		err = json.Unmarshal([]byte(result.(string)), &weather)
		require.NoError(t, err)
...
func TestGetWeatherTool_NewFunctionTool(t *testing.T) {
	...
	t.Run("AutoTool_Invocation", func(t *testing.T) {
		...
		args := `{"city": "Tokyo"}`
		result, err := GetWeatherToolNew.OnInvokeTool(ctx, rcw, args)
		require.NoError(t, err)
		...
		var weather Weather
		err = json.Unmarshal([]byte(resultStr), &weather)
		require.NoError(t, err)
...
func TestManualVsAutoGenerated_SchemaDifferences(t *testing.T) {
	...
	t.Run("FunctionalEquivalence", func(t *testing.T) {
		...
		manualResult, err1 := manualTool.OnInvokeTool(ctx, rcw, args)
		autoResult, err2 := autoTool.OnInvokeTool(ctx, rcw, args)

		require.NoError(t, err1)
		require.NoError(t, err2)

		// Both should return equivalent weather data
		var manualWeather, autoWeather Weather
		err1 = json.Unmarshal([]byte(manualResult.(string)), &manualWeather)
		err2 = json.Unmarshal([]byte(autoResult.(string)), &autoWeather)
...
func TestFunction_ErrorHandling(t *testing.T) {
	...
	t.Run("InvalidJSON", func(t *testing.T) {
		...
		args := `{"city": "Tokyo"`
		_, err := tool.OnInvokeTool(ctx, rcw, args)
		assert.Error(t, err)
	...
	t.Run("MissingRequiredField", func(t *testing.T) {
		...
		args := `{}`
		result, err := tool.OnInvokeTool(ctx, rcw, args)
		require.NoError(t, err)
